---
title: "Lateralization of phase-synchrony and amplitudes"
...
source(here("scripts","lateralization_of_amplitude_suppression_visual_cortex_functions.R" ))
```
...
The goal here is to produce a plot showing that our data replicate the classic result of having more alpha suppression in the contralateral visual cortex than in the ipsilateisual cortex during visuospatial attention.
ral v
Here we plot the log of the baseline corrected amplitudes for the low alpha frequency bands (i.e., between 6-9 Hz) which were extracted for each participant from GDM.
Amplitude modulations compared to baseline are averaged across the entire visual cortex as defined by the morphing with the yeo parcellation. 
...
system_assignements<-read.table(here("data","parcel_system_assignments.csv"), 
...
file_list<-list.files(here("data","parcel_amplitudes"), 
